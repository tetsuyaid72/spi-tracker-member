...
async def run_test():
    ...
    try:
        ...
        # -> Fill the email with a valid address (admin@admin.com) and submit the login form to reach the admin dashboard.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div[6]/div[2]/form/div/div/input').nth(0)
        ...
        # -> Click the 'Masuk' submit button to log in (element index 16).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div[6]/div[2]/form/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the visible login submit button to attempt to log in and reach the admin dashboard (then verify navigation).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div[6]/div[2]/form/div[2]/div/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        ...
        frame = context.pages[-1]
        assert await frame.locator("xpath=//*[contains(., 'admin@admin.com')]").nth(0).is_visible(), "The user list should show admin@admin.com after applying the search"
